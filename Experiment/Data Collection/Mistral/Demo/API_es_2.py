...
prompts = {
    "es": {
        "systemprompt": (
            "Este es un cuestionario de personalidad. Responda cada afirmación con un número "
            "del 1 al 5, donde el 1 representa que está definitivamente en desacuerdo con la afirmación y el 5 "
            "representa estar en total acuerdo con el contenido de la oración. Solo escriba un número por afirmación, "
            "no es necesaria explicación adicional."
        ),
        "userprompt": "f{itemtext}."
    }
}

def create_bfi_prompt(item_text, language="es"):
...
def process_full_survey(items, language="es"):
...
def generate_multiple_surveys(csv_path, num_surveys=100, language="es"):
...
if __name__ == "__main__":
    print("Mistral BFI-2 Generator (SPANISH)")
    INPUT_CSV = "BFI2Questionnaire.csv"
    OUTPUT_DIR = os.path.dirname(os.path.abspath(__file__))
    print(f"Working directory: {OUTPUT_DIR}")
    output_file = os.path.join(OUTPUT_DIR, "mistral_bfi2_results_es.json")
    print(f"--- ES ---")
    survey_results = generate_multiple_surveys(INPUT_CSV, num_surveys=100, language="es")
