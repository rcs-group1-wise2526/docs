...
prompts = {
    "en": {
        "systemprompt": (
            "This is a personality questionnaire. For each statement, respond with a single "
            "digit from 1 to 5, where 1 means strong disagreement and 5 means strong agreement. "
            "Respond ONLY with a single digit. Do not explain."
        ),
        "userprompt": "fI am someone who {itemtext}."
    }
}
...
def generate_multiple_surveys(csv_path, num_surveys=100, language="en"):
...
if __name__ == "__main__":
    print("Mistral BFI-2 Generator (ENGLISH)")
    INPUT_CSV = "BFI2Questionnaire.csv"
    OUTPUT_DIR = os.path.dirname(os.path.abspath(__file__))
    print(f"Working directory: {OUTPUT_DIR}")
    output_file = os.path.join(OUTPUT_DIR, "mistral_bfi2_results_en.json")
    print(f"--- EN ---")
    survey_results = generate_multiple_surveys(INPUT_CSV, num_surveys=100, language="en")
